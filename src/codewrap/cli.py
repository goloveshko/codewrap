...
def config_set(
    encoding: Optional[str] = typer.Option(None, help="Default tokenizer encoding (e.g. o200k_base, cl100k_base)"),
    exclude_binary: Optional[bool] = typer.Option(None, help="Auto-exclude binary files"),
    numbered: Optional[bool] = typer.Option(None, help="Auto-number duplicate output files"),
    cwd: Optional[bool] = typer.Option(None, help="Save outputs in execution directory"),
) -> None:
...
def main(
    ...
    files_list: Optional[Path] = typer.Option(
        None, "--files-list", "-f", help="Path to text file containing file paths to process"
    ),
    modified: bool = typer.Option(False, "--modified", "-m", help="Gather only Git modified/uncommitted files"),
    since: Optional[str] = typer.Option(None, "--since", "-s", help="Gather Git files modified since date/commit (e.g. '3 days ago')"),
    diff: bool = typer.Option(False, "--diff", "-d", help="Generate a Git unified diff context instead of full files"),
    ...
    bind: bool = typer.Option(
        False, "--bind", "-b", help="Bind the saved/loaded preset to current directory (Zero-Clutter)"
    ),
    init_config: bool = typer.Option(
        False, "--init-config", "-ic", help="Create a local .codewrap.json config file in current directory"
    ),
